	...
	if (query.constructor === Array) {
		...
		for (var i = 0; i < query.length; i++) {
			...
			if (ignore_counter-- > 0) {
				...
				if (query[i] && isObject(query[i]) && !insert_flag && !where_flag && update_flag) {
				...
				} else if (query[i] && isObject(query[i]) && !where_flag && !update_flag && insert_flag) {
				...
				} else if (query[i] && isObject(query[i]) && !insert_flag && !update_flag && where_flag) {
