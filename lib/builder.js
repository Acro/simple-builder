	...
	if (query.constructor === Array) {
		...
		for (var i = 0; i < query.length; i++) {
			...
			if (ignore_counter-- > 0) {
				...
				if (query[i] && query[i].constructor === Object && !insert_flag) {
					...
					keys.map(function (key) { 
						if (env != "mysql") {
							updates.push( key + "=$" + values_counter++ )
						}
				...
				} else if (query[i] && query[i].constructor === Object && insert_flag) {
